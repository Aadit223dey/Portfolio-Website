        ...
        <div className="flex items-center gap-5">
          {Object.entries(siteConfig.social).map(([platform, url]) => (
            <a
              ...
              aria-label={platform}
              className="p-2.5 rounded-xl bg-neutral-900 border border-border-premium text-text-secondary hover:text-white hover:border-accent-blue transition-all duration-300"
            >
              {socialIcons[platform] || <span className="text-[10px] font-mono uppercase">{platform}</span>}
            </a>
          ))}
        </div>
      </Container>
